...
// ── Sync student_state Bridge Table (Nafas → EduOS) ──────
// Writes Nafas mood/burnout fields to student_state on EduOS Supabase
const EDUOS_SUPABASE_URL = 'https://zuyizaiugpmhmeycqton.supabase.co';
const EDUOS_SUPABASE_KEY = process.env.EDUOS_SUPABASE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inp1eWl6YWl1Z3BtaG1leWNxdG9uIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkwODgyNDAsImV4cCI6MjA5NDY2NDI0MH0.FqOUqiR7GfttAEI8NY3bbOwFPnupxBsHMgYJCNT68PI';

async function eduosSupabaseFetch(path, method, body) {
  const opts = {
    method: method || 'GET',
    headers: {
      'apikey': EDUOS_SUPABASE_KEY,
      'Authorization': 'Bearer ' + EDUOS_SUPABASE_KEY,
      'Content-Type': 'application/json',
      'Prefer': method === 'POST' ? 'return=representation,resolution=merge-duplicates' : 'return=representation'
    }
  };
  if (body) opts.body = JSON.stringify(body);
  try {
    const res = await fetch(EDUOS_SUPABASE_URL + '/rest/v1/' + path, opts);
    if (!res.ok) return null;
    return await res.json();
  } catch (e) {
    console.error('[LEARN] EduOS Supabase error:', e.message);
    return null;
  }
}

async function syncNafasToStudentState(summaries) {
  if (!summaries || summaries.length === 0) return { synced: 0 };
  
  let synced = 0;
  
  // Group summaries by user to get latest mood
  const userMap = {};
  for (const s of summaries) {
    const uid = s.user_id || s.session_id;
    if (!uid || !s.school_id) continue; // Only sync school-linked users
    if (!userMap[uid]) userMap[uid] = s;
  }
  
  for (const [userId, summary] of Object.entries(userMap)) {
    // Determine mood and burnout from summary
    const moodSummary = summary.summary_text?.slice(0, 200) || null;
    const outcome = summary.outcome || 'unknown';
    const burnoutLevel = outcome === 'worsened' ? 'high' 
                       : outcome === 'same' ? 'medium' 
                       : outcome === 'improved' ? 'low' : null;
    const riskFlag = outcome === 'worsened' || (summary.key_topics || []).some(t => 
      t.includes('انتحار') || t.includes('إيذاء') || t.includes('أزمة')
    );
    
    await eduosSupabaseFetch('student_state', 'POST', {
      student_id: userId,
      school_id: summary.school_id,
      nafas_mood_summary: moodSummary,
      nafas_burnout_level: burnoutLevel,
      nafas_last_session_at: summary.created_at,
      nafas_risk_flag: riskFlag
    });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Sync to student_state bridge (school-linked users only) ──
    const syncResult = await syncNafasToStudentState(summaries || []);
    console.log('[LEARN] Student state sync:', syncResult);

    // Track schools processed
    const schoolsSeen = new Set((summaries || []).map(s => s.school_id).filter(Boolean));
