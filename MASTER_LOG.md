...
## [2026-09-21] إنجازات يوم 21 سبتمبر

### الهيدر الموحَّد:
- ✅ فقاعة الأسبوع شبه شفافة + نبضة خضراء + ساعة حية + زر رجوع دائري أحمر — مُطبَّق على جميع البوابات
- ✅ `platform-lang.js` يحقن زر اللغة + اسم المستخدم + `#header-tools` تلقائياً

### Analytics KPIs حقيقية (من DB):
- Staff = 118 | Students = 1,125 | Parents = 1,033 | Scheduled = 1,417

### بادج VARK:
- 225 سجل في DB ✅
- دعم MULTI = "متعدد الأنماط 🧠" ✅
- يظهر حتى إذا لم تؤدِ الاختبار (رمادي) ✅
- قابل للفرز ✅

### بوابة الطالبة:
- أُعيد بناؤها بالكامل مع Supabase حقيقي
- جدول اليوم من `schedules` DB مع أولوية معلمة حقيقية > شاغر
- الجمعة "نهاية الدوام" بعد ح4

### مولّد الجدول:
- إصلاح syntax error → 37 فصل تُحمَّل ✅

### بادج ولي الأمر:
- "لم يفتح البوابة بعد" (pending) + "غير مرتبط" (unlinked) ✅

### قواعد واجهة:
- أرقام إنجليزية في كل الواجهة (`toLocaleString('en-US')`) ✅
- "موبايل" بدل "جوال" ✅

---

## [2026-09-22] إنجازات يوم 22 سبتمبر

### حقن 1,240 سجل في `schedules`:
- من ملفات aSc الرسمية المعتمدة (37 فصل × 100%)
- الحلقة الأولى (G1-G4) + الحلقة الثانية (G5-G8) + الروضة

### إصلاح جدول الطالبة:
- أولوية معلمة حقيقية > شاغر ✅
- رفع limit من 10→30 ✅
- الجمعة "نهاية الدوام" بعد ح4 ✅

### تحديثات بيانات:
- أسماء المشهوري (590994) ✅
- هدى الطحان (567656) — الرقم الوزاري محدَّث ✅
- Li Chen (577036) — الاسم الرسمي محدَّث ✅

### نظام الاحتياط الذكي (مكتمل):
- 5 دوال DB: `auto_assign_substitute` + `check_substitute_alerts` + `v_staff_attendance_today` + `v_staff_cycle` + `calc_smart_work_hours`
- واجهة النائبة: بطاقة "خطة الاحتياط الذكية" + لوحة حضور حية ✅
- منطق التعيين: من الحاضرات فعلاً (`staff_checkin_log`) فقط ✅
- طوارئ: إذا 0 حاضرات بعد 7:15 → تنبيه فوري للنائبة + المديرة + أدمن ✅
- توقيت مُصحَّح: G1-G4 يبدأ 7:10 | G5-G8 يبدأ 8:00 ✅

### إصلاح الاكتشاف الذكي للغياب الجماعي:
- 3 مستويات: academic_calendar → school_news → يدوي
- تُحفظ النتيجة في `daily_attendance_status`
- إذا الغياب مبرَّر → خطة الاحتياط لا تُشغَّل ✅
- "لم تسجّل دخولاً بالباركود" بدل "غائبة" — لغة أدق ✅

### platform-workhours.js v2:
- Timeline متقطع: كل مسح QR بعد الاستئذان = فترة عمل مستقلة
- الاستئذان يقتطع من الـ8 ساعات
- تنبيه الموظفة بعد 10 د من انتهاء الاستئذان بدون مسح عودة
- تنبيه النائبة بعد 20 د
- pg_cron: `leave-return-alert` (كل 10 د 7:00-15:00 أحد-خميس) ✅

### نموذج الاستئذان:
- وقت المغادرة إلزامي ✅
- حد يومي 3 ساعات + حد شهري 6 ساعات ✅
- القفل بعد انتهاء الدوام ✅

### بوابة النائبة — إصلاحات:
- كلمة المرور: `VP@Aljood2026` (مُصلَّحة وتعمل) ✅
- بطاقات أدوات الجداول: 4 أعمدة بدل 3 + padding مضغوط ✅
- Core commit: `890edfc` | الجود commit: `c6f3484`

### DB — pg_cron جديد:
- `substitute-alert`: كل 5 دق 7:10-14:20 أحد-خميس ✅
- `duty-absence-alert`: كل 10 دق بعد 06:40 ✅
- `leave_requests auto-lock`: كل 15 دقيقة ✅
- `leave-return-alert`: كل 10 دق 7:00-15:00 ✅

---

## [2026-09-23] إنجازات يوم 23 سبتمبر

### مولّد الجدول — شاشة مقارنة قبل/بعد:
- Modal يظهر قبل زر النشر ✅
- يجلب الجدول الحالي من DB ويقارنه بالجديد
- يعرض: عدد الحصص المتغيّرة + الجديدة + غير المتغيّرة
- جدول تفصيلي: قبل/بعد لكل حصة (أول 20)
- زر "تأكيد النشر" → الحفظ الفعلي | زر "إلغاء" → العودة للمراجعة
- لا نشر بدون مراجعة ✅

### مراقبة الأخبار (تشغيل تلقائي 23 سبتمبر 06:00):
- 3 أخبار جديدة أُدخلت: ID 103 + 104 + 105
- تنبيه حرج: حداد رسمي في دبي 10 أيام (21 سبتمبر) — لا إغلاق مدارس رسمي
- بوابة النائبة تقرأ `school_news` تلقائياً عند الغياب الجماعي
